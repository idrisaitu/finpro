...
function setupNavigation() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Log the active class removal
            console.log('Removing active class from all links and sections');
...
function calculateLoan() {
    ...
    const loanTerm = parseInt(document.getElementById('loan-term-months').value) || 0;
    
    const monthlyRate = interestRate / 100 / 12;
    
    if (loanAmount <= 0 || monthlyRate <= 0 || loanTerm <= 0) {
        alert('Барлық қажетті өрістерді дұрыс мәндермен толтырыңыз.');
        return;
    }
...
function calculateSavings() {
    ...
    if (goal <= 0 || current < 0 || monthly <= 0 || interestRate < 0) {
        alert('Барлық қажетті өрістерді дұрыс мәндермен толтырыңыз.');
        return;
    }
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            updateIncomeList();
            updateDashboard();
            alert('Деректер сәтті импортталды!');
        } catch (error) {
            alert('Деректерді импорттау кезінде қате. Файл форматын тексеріңіз.');
        }
    };
    ...
    if (e.ctrlKey || e.metaKey) {
        switch(e.key) {
            case 's':
                e.preventDefault();
                saveToLocalStorage();
                alert('Деректер жергілікті түрде сақталды!');
                break;
            case 'e':
                e.preventDefault();
                exportData();
